---
applyTo: '**/*.al'
description: Imported BCQuality rule from microsoft/knowledge/style/no-else-after-terminating-statement.md
---

# Omit `else` when the `then` branch ends with `exit`, `break`, `skip`, `quit`, or `error`

Source: microsoft/knowledge/style/no-else-after-terminating-statement.md
