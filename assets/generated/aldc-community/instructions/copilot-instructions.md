...
| **Lean, spec-kit-aligned feature loop (LOW–MEDIUM)?** | **`@Ink, AL Lean SDD`** | **Constitution → Spec → Implement → Test → Docs → Finalise (lower token cost)** |
...
New feature (LOW)?         → al-spec.create → @Phil, AL Developer
                             OR → @Ink, AL Lean SDD (if lean-SDD is set up)
...
Lean SDD feature?          → @Ink, AL Lean SDD
```

### Lean SDD vs Full ALDC, Decision Guide

| Dimension | Use `@Ink, AL Lean SDD` | Use Full ALDC (Conductor) |
|-----------|-------------------|---------------------------|
...
1. `@Angus, AL Architect`, Designs solution, creates `specs/Plans/YYYY-MM-DD-{req_name}/{req_name}.architecture.md`
...
3. `@Malcolm, AL Conductor`, Reads spec + architecture from `specs/Plans/YYYY-MM-DD-{req_name}/`, orchestrates TDD: planning → implementation → review
...
1. `@Ink, AL Lean SDD`, Run `/speckit.constitution` once, then the 5-step feature loop per requirement under `specs/SDD/YYYY-MM-DD-{slug}/`
