...
| `Could not resolve dependency '<Main App>'` | Main app from `app/` is not installed in the container, or symbols in `test\.alPackages` are stale | Publish `app/` first, or rerun `.AL-Go\localDevEnv.ps1`. Delete `test\.alPackages\` to force a re-download |
...
| Main app shows `IsPublished=True IsInstalled=False` after a previous broken run | A prior `Publish-BcContainerApp -upgrade` left the app published but uninstalled | The script auto-recovers by calling `Sync-BcContainerApp` + `Install-BcContainerApp` in step 3. To do it manually: `Sync-BcContainerApp -containerName bc-28 -tenant default -appName '<Main App>' -force` then `Install-BcContainerApp` with the same args |
