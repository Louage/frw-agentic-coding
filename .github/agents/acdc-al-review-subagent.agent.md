...
6. **Custom-layer BCQuality (customer/partner forks).** If any layer is installed via `acdc.bcquality.customLayers` (extension setting; content lives in the extension's per-user globalStorage under `bcquality-custom/<layer-id>/`), consult those rules **in addition to** the bundled Microsoft leaves. Discover them via the language-model tools `acdc_list_bcquality_custom_rules` (metadata) and `acdc_get_bcquality_custom_rule` (full body); the equivalents `acdc_list_bcquality_custom_skills` / `acdc_get_bcquality_custom_skill` expose action skills. Every imported rule and skill carries a mandatory `<layer-id>__` prefix so it cannot shadow a bundled name. Priority when guidance conflicts: **custom > community > microsoft** (customer's own rules override community forks, which override the upstream Microsoft baseline). Cite these findings with `source: "bcquality"` and put the qualified rule path (e.g. `custom/knowledge/<layer-id>__<slug>.instructions.md`) in `references[0].path`; `validate-evidence` accepts custom-layer citations without CI clone resolution because the fork is not available to CI (the provenance.json produced by the sync engine is the authoritative record).
