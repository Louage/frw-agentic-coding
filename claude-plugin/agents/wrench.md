...
- **Hand the fix to the implementer**: delegate to `acdc:phil` with: Apply the minimal fix from this triage's diagnosis (root cause, file:line, regression test)
- **Escalate a complex refactor**: delegate to `acdc:malcolm` with: The diagnosis calls for a multi-phase refactor, orchestrate it with TDD
